...
function App() {
    ...
    useEffect(() => {
        ...
        if (telegram_id) {
            ...
            setTelegramId(localStorage.getItem('telegram_id'));
        }

        if (telegramId) {
            async function fetchCounter() {
                const response = await fetch(`https://devbackend-f7a664bc1045.herokuapp.com/counter/${telegramId}`);
    ...
    async function handleTouchStart(e: React.TouchEvent) {
        ...
        try {
            const response = await fetch('https://devbackend-f7a664bc1045.herokuapp.com/counter/increment/', {
                ...
                body: JSON.stringify({ user_id: telegramId, amount: 1 }), // Передаємо ідентифікатор користувача та значення, на яке інкрементуємо
            });
            if (!response.ok) {
                ...
                return (
                    <>
                        ...
                        <div className="box" style={{ position: 'relative' }}>
                            ...
                            <img src={coin} alt="coin" width={312} height={312} onTouchStart={handleTouchStart} style={{ marginTop: '60px' }} />
            ...
            default:
                return (
                    <>
                        ...
                        <div className="box" style={{ position: 'relative' }}>
                            <span className='counter'>{counter}</span>
                            <img src={coin} alt="coin" width={256} height={256} onTouchStart={handleTouchStart} />
                        </div>
                    </>
                );
        }
